...
import observer5.observer.Observador;
import observer5.observer.ObservadorConsole;
import observer5.observer.ObservadorGrafico;
...
public class ControleDeposito extends javax.swing.JFrame {
    ...
    public void notificarObservadores() {
        try {
            String strQtdAtual = jtfQtdade.getText().trim();
            String strQtdCritica = jtfQtdadeCritica.getText().trim();
            String strQtdMaxima = jtfQtdadeMax.getText().trim();

            // Validar valores antes de tentar convertê-los
            if (strQtdAtual.isEmpty() || strQtdCritica.isEmpty() || strQtdMaxima.isEmpty()) {
                throw new IllegalArgumentException("Todos os campos devem ser preenchidos com valores numéricos.");
            }

            int qtdAtual = Integer.parseInt(strQtdAtual);
            int qtdCritica = Integer.parseInt(strQtdCritica);
            int qtdMaxima = Integer.parseInt(strQtdMaxima);

            // Notificar os observadores
            for (Observador obs : observadores) {
                obs.atualizar(qtdAtual, qtdCritica, qtdMaxima);
            }
        } catch (NumberFormatException e) {
            // Captura erros de formato numérico
            javax.swing.JOptionPane.showMessageDialog(this, "Os campos devem conter apenas valores numéricos inteiros.", "Erro de Entrada", javax.swing.JOptionPane.ERROR_MESSAGE);
    ...
    public ControleDeposito() {
        ...
        ObservadorConsole observadorConsole = new ObservadorConsole();
        ObservadorGrafico observadorGrafico = new ObservadorGrafico(jLabel5); // Atualiza o jLabel5, por exemplo

        // Registro dos observadores
        adicionarObservador(observadorConsole);
        adicionarObservador(observadorGrafico);
